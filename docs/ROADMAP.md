...
2026-01-23 이후 멈췄던 프로젝트를 다시 점검한 결과입니다. 아래 Phase 0~3의 일정(2025-11 ~ 2026-01)은 모두 지났습니다. [ADR 0003](./adr/0003-continue-existing-codebase.md)에 따라 **현 코드베이스를 정리하며 지속**하고, 기존 Phase 일정은 아래 정리 항목의 순서를 정할 때까지 보류합니다.

| 항목 | 상태 | 비고 |
|------|------|------|
| 현 코드베이스를 이어갈지, 핵심 기능만 새 프로젝트로 옮길지 결정 | ✅ 완료 | 정리 후 지속 (ADR 0003) |
| 유출된 Supabase 키 폐기 및 저장소에서 제거 | ✅ 완료 | #74 |
| 정리 항목의 순서와 첫 트랙 계획 | 🔜 다음 작업 | 아래 📋 항목 대상 |
| 보안 후속 조치 (서버 액션·API 인가, DB 권한·RLS 재정비) | 📋 계획됨 | 상세는 비공개 점검 보고서 |
| Next.js 14 · Node 20 지원 종료 대응 | 📋 계획됨 | Next 15 이상은 React 19 필요 |
| DB 마이그레이션 기준선 재수립 (운영 DB 스키마 기준) | 📋 계획됨 | |
| HiStudy 데모 페이지·컴포넌트 삭제 | 📋 계획됨 | ADR 0003에서 추가 |
| 저장소 정리·CI 복구 (서식, 줄바꿈 정규화, 추적 산출물) | 📋 계획됨 | |
| 완료됐지만 남아 있는 work-plan 정리 | 📋 계획됨 | histudy-demo-cleanup, youtube-ai-summarization |
