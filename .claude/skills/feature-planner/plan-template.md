...
5. 📝 Log learnings in [DEV_LOG](../logs/2025-Q1.md) (1-2 lines, 자산화 가치 → Library 승격)
...
> 📝 Phase 완료 시 아래 학습 내용을 `docs/logs/YYYY-Q1.md`에 복사
> 자산화 가치 있는 항목 → Library 승격

| Date | Tag | Learning |
|------|-----|----------|
| YYYY-MM-DD | [tag] | 학습 내용 1-2줄 |

### Critical Decisions (이 PLAN에서만)
- [아키텍처 변경, 일정 지연 등 중요 결정만 기록]
...
### Issue-PR 연결
PR 생성 시 `Closes #XX` 포함 → Issue 자동 닫힘
```bash
gh pr create --title "feat: description - Closes #XX"
```
