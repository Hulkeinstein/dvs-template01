...
test.describe('Payment Security API Tests', () => {
  ...
  test('should reject order creation with tampered price', async () => {
    ...
    // 2. Send request
    const response = await authContext.post('/api/payment/paypal/create-order', {
      data: payload,
    });
    ...
    if (body.code === 'PRICE_TAMPERED') {
        ...
        console.warn('Course likely not found in DB even if scraped from UI (or fallback used).');
    }
  });
