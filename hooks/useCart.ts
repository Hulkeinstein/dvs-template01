...
export const useCart = () => {
  ...
  const total_items = useSelector((state: RootState) => state.CartReducer.total_items);
  const total_amount = useSelector((state: RootState) => state.CartReducer.total_amount);

  // 사용자별 카트 키 생성
  const cartKey = useMemo(() =>
    getCartKey(session?.user?.id),
    [session]
  );

  // 카트 저장 (Redux만 업데이트 - CartProvider가 localStorage 처리)
  const saveCart = useCallback((nextCart?: any[]) => {
  ...
  // 수량 조절
  const toggleAmount = useCallback((id: string, value: "inc" | "dec") => {
