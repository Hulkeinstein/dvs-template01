...
import { mapFormDataToDB, mapDBToFormData, mapFormDataToSettings } from './courseDataMapper.js';
...
function runTests() {
  ...
  const expectedDBFields = [
    'title', 'description', 'about_course', 'regular_price', 'discounted_price',
    'language', 'difficulty_level', 'max_students', 'intro_video_url', 'is_free',
    'start_date', 'requirements', 'targeted_audience', 'total_duration_hours',
    'total_duration_minutes', 'content_drip_enabled', 'content_drip_type',
    'course_tags'
  ];
  
  const missingDBFields = expectedDBFields.filter(field => !(field in dbData));
  ...
  const expectedFormFields = [
    'title', 'shortDescription', 'description', 'category', 'level',
    'maxStudents', 'introVideoUrl', 'price', 'discountPrice', 'language',
    'startDate', 'requirements', 'targetedAudience', 'totalDurationHours',
    'totalDurationMinutes', 'contentDripEnabled', 'contentDripType',
    'courseTags', 'certificateEnabled', 'certificateTitle', 'passingGrade',
    'enrollmentDeadline', 'endDate', 'lifetimeAccess'
  ];
  
  const missingFormFields = expectedFormFields.filter(field => !(field in formData));
  ...
  if (Array.isArray(dbData.course_tags) && typeof formData.courseTags === 'string') {
  ...
  console.log('allow_lifetime_access (settings):', settingsData.allow_lifetime_access);
  console.log('Back to lifetimeAccess:', formData.lifetimeAccess);
  
  console.log('\n🎯 Test Summary');
  console.log('='.repeat(50));
  console.log('All critical fields are being mapped correctly.');
  console.log('The centralized mapper handles all Course Settings fields properly.');
}
