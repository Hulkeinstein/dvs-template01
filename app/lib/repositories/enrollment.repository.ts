...
export class EnrollmentRepository extends BaseRepository<Enrollment> {
   ...
   * 수료증 발급
   */
  async issueCertificate(enrollmentId: string, certificateUrl: string): Promise<Enrollment> {
  ...
  async getStats(userId: string): Promise<{
    ...
    return {
      ...
      inProgress: enrollments.filter(e => !e.completed_at && e.progress > 0).length,
      notStarted: enrollments.filter(e => !e.completed_at && e.progress === 0).length
    };
  }
}
