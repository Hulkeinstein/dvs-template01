...
import { extractYouTubeId, isValidYouTubeUrl, getCanonicalUrl } from '@/app/lib/utils/youtube';
import { cache } from 'react';

// 1. oEmbed Extraction (No API Key required)
const fetchOEmbed = cache(async (videoId: string): Promise<YouTubeOEmbedResponse> => {
...
// 2. Data API Extraction (API Key required, Duration only)
const fetchDuration = cache(async (videoId: string): Promise<number | null> => {
  ...
  try {
    ...
    return durationIso ? parseDuration(durationIso) : null;
  } catch (err) {
...
// 3. Duplicate Check
export async function checkYoutubeDuplicate(courseId: string, youtubeId: string): Promise<boolean> {
...
export async function fetchYouTubeMetadata(youtubeUrl: string): Promise<{
  ...
  if (!isValidYouTubeUrl(youtubeUrl)) {
    return { success: false, error: '유효하지 않은 YouTube URL입니다.', errorType: 'INVALID_URL' };
  }

  const videoId = extractYouTubeId(youtubeUrl);
  if (!videoId) {
    return { success: false, error: 'YouTube ID를 추출할 수 없습니다.', errorType: 'INVALID_URL' };
  }

  try {
    // Parallel fetch for speed
    const [oEmbedData, durationSeconds] = await Promise.all([
        fetchOEmbed(videoId),
    ...
    if (error.message === 'PRIVATE_OR_DELETED') {
      return { success: false, error: '비공개이거나 삭제된 영상입니다.', errorType: 'PRIVATE_OR_DELETED' };
    }
    if (error.message === 'RATE_LIMIT') {
      return { success: false, error: '요청이 너무 많습니다. 잠시 후 시도해주세요.', errorType: 'RATE_LIMIT' };
    }

    return { success: false, error: '영상 정보를 가져오는 중 오류가 발생했습니다.', errorType: 'UNKNOWN' };
  }
}
