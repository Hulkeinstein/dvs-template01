...
export async function createLesson(lessonData) {
  try {
    ...
    if (!session?.user?.email) {
      return {
        success: false,
        error: 'You must be logged in to create lessons',
      };
    }
...
export async function updateLesson(lessonId, updates) {
  try {
    ...
    if (!session?.user?.email) {
      return {
        success: false,
        error: 'You must be logged in to update lessons',
      };
    }
...
export async function deleteLesson(lessonId) {
  try {
    ...
    if (!session?.user?.email) {
      return {
        success: false,
        error: 'You must be logged in to delete lessons',
      };
    }
...
export async function reorderLessons(courseId, lessonOrders) {
  try {
    ...
    if (!session?.user?.email) {
      return {
        success: false,
        error: 'You must be logged in to reorder lessons',
      };
    }
    ...
    if (!course || course.instructor_id !== userData.id) {
      return {
        success: false,
        error: 'You do not have permission to reorder lessons in this course',
      };
    }
    ...
    const updatePromises = lessonOrders.map(({ id, order }) =>
      supabase.from('lessons').update({ sort_order: order }).eq('id', id)
    );
