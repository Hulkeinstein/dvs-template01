...
export async function getInstructorDashboardData(instructorId) {
  ...
  try {
    ...
    if (courseIds.length === 0) {
      return {
        totalStudents: 0, totalCourses: 0, totalEarnings: 0, 
        activeCourses: 0, enrolledCourses: 0, completedCourses: 0
      };
    }
    ...
    const { count: enrolledCourses } = await supabase.from('enrollments').select('id', { count: 'exact', head: true }).in('course_id', courseIds);
