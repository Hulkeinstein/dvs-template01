...
import { TranscriptResponse, SummaryErrorType, TranscriptResponseSchema } from '@/types/summary';
...
export async function fetchTranscript(youtubeUrl: string): Promise<TranscriptResult> {
  try {
    ...
    if (!supadata) {
      return { success: false, error: 'Missing Supadata API Key', errorType: 'API_ERROR' };
    }
    ...
    if (!videoId) {
      return { success: false, error: 'Invalid YouTube URL', errorType: 'UNKNOWN' };
    }

    // 3. Call Supadata API
    const response = await fetch(`https://api.supadata.ai/v1/youtube/transcript?url=${encodeURIComponent(youtubeUrl)}&text=false`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        return { success: false, error: 'Rate limit exceeded', errorType: 'RATE_LIMIT' };
      }
      if (response.status === 404 || response.status === 500) {
        // Supadata returns 500 often for "no transcript found" or really private videos
        // 404 is more explicit 
         return { success: false, error: 'No transcript found or API error', errorType: 'NO_TRANSCRIPT' };
      }
      return { success: false, error: `API Error: ${response.status} ${response.statusText}`, errorType: 'API_ERROR' };
    }
    ...
    if (json?.error) {
      console.error('Supadata API Error Response:', JSON.stringify(json, null, 2));
      ...
      if (errorMessage.toLowerCase().includes('transcript') ||
        ...
        return { success: false, error: '이 영상에는 자막이 없습니다.', errorType: 'NO_TRANSCRIPT' };
      }

      if (errorMessage.toLowerCase().includes('rate') ||
          errorMessage.toLowerCase().includes('limit')) {
        return { success: false, error: 'API 요청 한도 초과', errorType: 'RATE_LIMIT' };
      }
    ...
    if (!parseResult.success) {
      ...
      console.error('Errors:', JSON.stringify(parseResult.error.errors, null, 2));

      return { success: false, error: 'Invalid API response format', errorType: 'API_ERROR' };
    }
