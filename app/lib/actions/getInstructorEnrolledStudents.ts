 ...
 */
export async function getInstructorEnrolledStudents(): Promise<GetEnrolledStudentsResponse | ErrorResponse> {
  try {
    // Get current user session
    const session = await getServerSession(authOptions);
    
    if (!session?.user?.email) {
      return { error: 'Unauthorized', message: 'You must be logged in to view enrolled students' };
    }
    ...
    if (coursesError) {
      ...
      return { error: 'Failed to fetch courses', message: coursesError.message };
    }
    ...
    if (enrollmentsError) {
      ...
      return { error: 'Failed to fetch enrollments', message: enrollmentsError.message };
    }

    // Transform the data into the expected format
    const enrolledStudents: EnrolledStudent[] = (enrollments || []).map(enrollment => ({
      enrollment: {
        id: enrollment.id,
      ...
      student: enrollment.user as StudentProfile || {
      ...
      course: enrollment.course as CourseInfo || {
 ...
 * @returns Promise with enrolled students data and summary statistics
 */
export async function getEnrolledStudentsByCourse(courseId: string): Promise<GetEnrolledStudentsResponse | ErrorResponse> {
  try {
    // Get current user session
    const session = await getServerSession(authOptions);
    ...
    if (courseError || !course) {
      return { error: 'Course not found', message: 'The specified course does not exist' };
    }

    if (course.instructor_id !== instructorId) {
      return { error: 'Unauthorized', message: 'You can only view students for your own courses' };
    }
