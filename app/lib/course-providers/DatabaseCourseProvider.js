...
export class DatabaseCourseProvider extends CourseProvider {
  ...
  async getCourseById(courseId) {
    try {
      ...
      if (this.session && (result.course.status === 'draft' || result.course.status === 'pending')) {
        const isOwner = this.session.user?.email && 
  ...
  transformCourse(rawData) {
    return {
      ...
      discount: rawData.discounted_price && rawData.regular_price > rawData.discounted_price
        ? Math.round(((rawData.regular_price - rawData.discounted_price) / rawData.regular_price) * 100)
        : 0,
      ...
      date: rawData.updated_at ? new Date(rawData.updated_at).toLocaleDateString() : '',
      ...
      courseContent: [{
        ...
        contentList: rawData.lessons && rawData.lessons.length > 0 
          ? [{
      ...
      courseInstructor: rawData.instructor ? [{
        ...
        body: [{
          ...
          img: rawData.instructor.avatar_url || "/images/client/avatar-02.png",
          ...
          studentNumber: "0",
          course: "1",
          desc: rawData.instructor.bio || "Experienced instructor passionate about teaching.",
          social: [
            { icon: "facebook", link: "#" },
            { icon: "twitter", link: "#" },
            { icon: "linkedin", link: "#" }
      ...
      roadmap: [
        {
          text: "Start Date",
          desc: rawData.start_date ? new Date(rawData.start_date).toLocaleDateString() : "Flexible"
        },
        {
          text: "Enrolled",
        ...
        {
          text: "Certificate",
          desc: rawData.course_settings?.[0]?.certificate_enabled ? "Yes" : "No"
        }
      ],
