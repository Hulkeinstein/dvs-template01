...
interface QuestionProps {
  ...
  editor?: unknown;
  answerEditor?: unknown;
  currentQuestion: InternalQuestion;
  setCurrentQuestion: (question: InternalQuestion) => void;
  isEditing?: boolean;
}
...
const Question: React.FC<QuestionProps> = ({
  ...
  const currentQuestion = useMemo<InternalQuestion>(() => {
  ...
  const [displayFormat, setDisplayFormat] = useState<'text' | 'image' | 'both'>(
    'text'
  );
  ...
  const [blanks, setBlanks] = useState<BlankAnswer[]>(
  ...
  const [sortItems, setSortItems] = useState<SortItem[]>(
    () => currentQuestion?.sortItems || []
  );
  const [newSortItem, setNewSortItem] = useState<string>('');
  ...
  const [newLeftItem, setNewLeftItem] = useState<string>('');
  const [newRightItem, setNewRightItem] = useState<string>('');
  ...
  useEffect(() => {
    if (
      selectedOption === 'Fill in the Blanks' ||
  ...
  useEffect(() => {
    ...
    currentQuestion?.sortItems?.length,
  ]);
  ...
  const handleAddOption = (): void => {
    ...
    setCurrentQuestion({
      ...
      correctAnswer:
        ...
        (selectedOption === 'Single Choice' ? newOption.id : null),
    });
  ...
  const handleSetCorrectAnswer = (optionId: string): void => {
    if (
      ...
      const currentAnswers = Array.isArray(currentQuestion.correctAnswer)
        ? (currentQuestion.correctAnswer as (string | number)[])
        : [];
  ...
  const addBlank = (): void => {
    ...
    setCurrentQuestion({
      ...
      blanks: newBlanks,
      correctAnswer: {
        ...(currentQuestion.correctAnswer as Record<number, string[]>),
        [newBlankId]: [],
      },
    });
  };

  const removeBlank = (blankId: number): void => {
  ...
  const addAnswerToBlank = (blankId: number, answer: string): void => {
  ...
  const updateBlankSettings = (
    blankId: number,
    settings: Partial<BlankAnswer>
  ): void => {
    const updatedBlanks = blanks.map((blank) =>
  ...
  const handleOptionImageUpload = (e: ChangeEvent<HTMLInputElement>): void => {
    const file = e.target.files?.[0];
    if (file && file.type.startsWith('image/')) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setOptionImage(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const removeSortItem = (itemId: number): void => {
  ...
  const moveSortItem = (fromIndex: number, toIndex: number): void => {
      ...
      <div className="content">
        ...
        <div className="course-field mb--20">
          ...
          <div className="rbt-create-course-thumbnail upload-area">
            ...
            <small>
              <i className="feather-info"></i> Upload an image to display with
              your question
            </small>
          </div>
        </div>
            ...
            {(selectedOption === 'Single Choice' ||
              ...
              <div className="form-check form-switch">
                <input
                  ...
                  checked={currentQuestion.randomize || false}
                  onChange={(e) =>
                    setCurrentQuestion({
                      ...currentQuestion,
          ...
          >
            <div className="mb--20">
              ...
              <small className="d-block mb-2">
                <i className="feather-info"></i> Add items in the correct order.
                Students will need to arrange them correctly.
              </small>
            ...
            {sortItems.length > 0 && (
              <div className="mt--20">
                ...
                <small className="text-muted mt-2 d-block">
                  <i className="feather-info"></i> The order shown here is the
                  correct answer
                </small>
              </div>
            )}
            ...
            {(displayFormat === 'image' || displayFormat === 'both') && (
              <div className="course-field mt--20">
                ...
                <div className="rbt-create-course-thumbnail upload-area">
                  ...
                  <small>
                    <i className="feather-info"></i> Upload an image for this
                    answer option
                  </small>
                </div>
              </div>
                ...
                <div className="rbt-accordion-style rbt-accordion-04">
                  {currentQuestion.options.map((option, index) => {
                    ...
                    return (
                      <div
                        key={optionId}
                        className="rbt-accordion-item card mb--10"
                      >
                            ...
                            <div className="d-flex align-items-center gap-3">
                              {(selectedOption === 'Single Choice' ||
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    handleSetCorrectAnswer(optionId)
                                  }
                                  style={{ minWidth: '40px' }}
                                >
                                  <i
                              ...
                              {(selectedOption === 'Multiple Choice' ||
                                ...
                                <button
                                  ...
                                  className={`btn btn-sm ${
                                    (
                                      currentQuestion.correctAnswer as (
                                        | string
                                        | number
                                      )[]
                                    )?.includes(optionId) ||
                              ...
                              {typeof option === 'object' &&
                                (option as QuestionOption & { image?: string })
                                  .image &&
                                (displayFormat === 'image' ||
                                  displayFormat === 'both') && (
                                  <Image
        ...
        >
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) => {
                    ...
                    if (file && file.type.startsWith('image/')) {
                      ...
                      reader.onloadend = () => {
                        setCurrentQuestion({
                          ...currentQuestion,
                          imageMatchingImage: reader.result as string,
                        });
                      };
                      reader.readAsDataURL(file);
                    }
          ...
          <div className="content">
            ...
            <input
              ...
              onChange={(e) =>
                setCurrentQuestion({
                  ...currentQuestion,
                  imageMatchingText: e.target.value,
                })
              }
            />
          </div>
          <button
            ...
            onClick={() => {
              if (
                currentQuestion.imageMatchingImage &&
                currentQuestion.imageMatchingText
              ) {
                const newPair: ImageMatchingPair = {
                  id: Date.now().toString(),
                ...
                setCurrentQuestion({
                  ...
                  imageMatchingText: '',
                  correctAnswer: [...pairs, newPair].reduce(
          ...
          {currentQuestion.imageMatchingPairs &&
            currentQuestion.imageMatchingPairs.length > 0 && (
              <div className="mt--20">
                <h6>Added Image-Text Pairs</h6>
                {currentQuestion.imageMatchingPairs.map((pair, index) => (
                  <div
                    key={pair.id}
                    className="d-flex align-items-center gap-3 mb--10 p-3 border rounded"
                  >
                    <Image
                      src={pair.image}
                      width={60}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setCurrentQuestion({
                          ...
                          correctAnswer: updatedPairs.reduce(
        ...
        <div
          className={`course-field mt--20 ${
            selectedOption === 'Matching' || currentQuestion.type === 'Matching'
              ? 'd-block'
              : 'd-none'
          }`}
        >
          ...
          <small className="d-block mb-3">
            <i className="feather-info"></i> Create items for the left and right
            columns. Students will match items from left to right.
          </small>
                        ...
                        matchingPairs: {
                          ...
                          correctMatches:
                            (currentQuestion.matchingPairs &&
                              currentQuestion.matchingPairs.correctMatches) ||
                            {},
                        },
                      });
                    }
              ...
              {matchingLeftItems.map((item) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="rbt-btn-close"
                    onClick={() => {
                      const updatedItems = matchingLeftItems.filter(
                        (i) => i.id !== item.id
                      );
              ...
              {matchingRightItems.map((item) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="rbt-btn-close"
                    onClick={() => {
                      const updatedItems = matchingRightItems.filter(
                        (i) => i.id !== item.id
                      );
          ...
          {matchingLeftItems.length > 0 && matchingRightItems.length > 0 && (
            <div className="mt-4">
              ...
              <small className="d-block mb-3">
                <i className="feather-info"></i> Select the correct match for
                each left item.
              </small>
        ...
        >
          <div className="mb--20">
            ...
            <small className="d-block mb-2">
              <i className="feather-info"></i> Write your question and click
              &quot;Add Blank&quot; to insert blank spaces. Blanks will appear
              as [1], [2], etc.
            </small>
            <div className="d-flex gap-2 align-items-center">
              <input
              ...
              {blanks.map((blank) => (
                <div key={blank.id} className="card mb--20 p-3">
                  ...
                  {blankAnswers[blank.id] &&
                    blankAnswers[blank.id].length > 0 && (
                      <div className="mb-3">
                        ...
                        {blankAnswers[blank.id].map((answer, ansIdx) => (
                          <div
                            key={ansIdx}
                            className="d-inline-flex align-items-center gap-2 me-2 mb-2"
                          >
                            ...
                            <button
                              ...
                              style={{ lineHeight: 1 }}
                              onClick={() =>
                                removeAnswerFromBlank(blank.id, ansIdx)
                              }
                            >
                              <i className="feather-x"></i>
                            </button>
