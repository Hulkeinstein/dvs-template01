...
describe('Summary Actions', () => {
  ...
  describe('generateKeySummary', () => {
    ...
    it('should fail if token limit is exceeded', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errorType).toBe('VIDEO_TOO_LONG');
    });
  ...
  describe('generateFullSummary', () => {
    ...
    it('should generate full summary successfully', async () => {
        ...
        const keySummaryResponse = {
            choices: [{ message: { content: JSON.stringify({ key_notes: ["Key 1"] }) } }],
            usage: { total_tokens: 100, prompt_tokens: 50, completion_tokens: 50 }
        };
        const detailedNotesResponse = {
            choices: [{ message: { content: JSON.stringify({ detailed_notes: [] }) } }],
            usage: { total_tokens: 200, prompt_tokens: 100, completion_tokens: 100 }
        };

        (global.fetch as jest.Mock)
            .mockResolvedValueOnce({ ok: true, json: async () => keySummaryResponse })
            .mockResolvedValueOnce({ ok: true, json: async () => detailedNotesResponse });

        const result = await generateFullSummary(mockTranscriptResponse, "Description", 100);
