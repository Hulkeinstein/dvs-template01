import { fileToBase64, base64ToBlob, validateFileSize, validateFileType } from '@/app/lib/utils/fileUpload';
...
describe('File Upload Utilities', () => {
    ...
    it('should reject if file reading fails', async () => {
      ...
      global.FileReader = jest.fn(() => ({
        ...
        addEventListener: function(event, handler) {
          if (event === 'error') {
            setTimeout(() => handler({ target: { error: new Error('Read error') } }), 0);
          }
        }
      }));
  ...
  describe('validateFileSize', () => {
    it('should return true for file within size limit', () => {
      const mockFile = new File(['a'.repeat(1024 * 1024)], 'test.jpg', { type: 'image/jpeg' });
