...
describe('SummaryDisplay', () => {
  ...
  it('renders nothing if no data, no loading, no error', () => {
    const { container } = render(
      <SummaryDisplay data={null} isLoading={false} />
    );
    expect(container).toBeEmptyDOMElement();
  });

  it('shows loading message', () => {
    render(<SummaryDisplay data={null} isLoading={true} />);
    expect(
      screen.getByText(
        /자막을 추출하고 있습니다|분석하고 있습니다|요약을 생성하고 있습니다/
      )
    ).toBeInTheDocument();
  });
  ...
  it('renders key notes', () => {
    render(<SummaryDisplay data={mockData} isLoading={false} />);
