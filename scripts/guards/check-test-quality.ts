...
class TestQualityChecker {
  ...
  checkForDirectSupabaseCalls(): CheckResult {
    ...
    try {
      ...
      if (result) {
        this.warnings.push('Found direct Supabase imports in tests (should use repository pattern):\n' + result);
        return {
  ...
  checkTestCoverage(): CheckResult {
    ...
    if (!fs.existsSync(coveragePath)) {
      console.log(`${YELLOW}⚠️  No coverage report found. Run 'npm test:coverage' first${RESET}`);
      return {
        passed: true,
    ...
    try {
      ...
      for (const [key, threshold] of Object.entries(thresholds)) {
        ...
        if (actual < threshold) {
          passed = false;
          failures.push(`${key}: ${actual.toFixed(2)}% (threshold: ${threshold}%)`);
        }
      }
  ...
  checkForConsoleLog(): CheckResult {
    ...
    try {
      ...
      if (result) {
        ...
        if (lines > 5) {
          this.warnings.push(`Found ${lines} console.log statements in production code`);
          return {
            passed: true,
  ...
  checkNodeVersion(): CheckResult {
    ...
    if (major !== 20 || minor < 11) {
      this.errors.push(`Node version must be 20.11.x or higher (current: ${nodeVersion})`);
      return {
        passed: false,
    ...
    if (major > 20) {
      this.warnings.push(`Node version ${nodeVersion} is higher than tested version (20.x)`);
      return {
        passed: true,
